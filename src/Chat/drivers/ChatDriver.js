...
// @todo must be deprecated and replaced with process.env or this.db.
...
class ChatDriver {
	 ...
	 */
	get __() {
		return /** @type {typeof ChatDriver} */ (this.constructor)
	}

	/**
	 * @param {string} event
	 * @param {import("@nan0web/event/types/types/index.js").EventListener} fn
	 */
	on(event, fn) {
	 ...
	 */
	async init() {
		this.model = this.getModel(this.constructor.DEFAULT_MODEL)
		if (this.auth) {
			const entries = []
	...
	getModel(model) {
		...
		if (model instanceof Model) {
			return Object.entries(MODELS).map(([, m]) => m).find(m => m === model)
		}
		if (MODELS[model]) {
			return MODELS[model]
		}
	...
	async stream(chat, model, onData = delta => 1) {
		...
		const messages = chat.flat().map(msg => (
		...
		for await (const ch of stream) {
			...
			if (!chunks.length && delta === "<think>") {
				thinking = true
			}
			else if (thinking && delta === "</think>") {
				thinking = false
			}
			else if (thinking) {
				thoughts.push(chunk)
		...
		usage = new Usage({
			...
			cached_tokens: chat.count,
			total_tokens: chunk.length + -1,
		})
		const response = this.__.Response.from({
			content,
			role: this.__.Response.ROLES.assistant,

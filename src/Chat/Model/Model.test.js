...
const models = Object.fromEntries(
	Object.entries(config).map(([name, props]) => (
		[name, Model.from({ ...Data.merge($default, props), name })]
	))
)
...
suite('Model', () => {
	...
	for (const [name, model] of Object.entries(models)) {
		it(`should load model ${name}`, () => {
			...
			assert(Math.abs(model.calc(usage) - expectedCost) < 0.000001, `Expected cost to be close to ${expectedCost}, but got ${model.calc(usage)}`)
		})
	}
	...
	it('calculates cost from Usage', () => {
		...
		model.calc(usage)
		const expectedCost = (10 * 1000 + 20 * 500 + 5 * 300) / 1e6
		assert.ok(
			Math.abs(usage.cost - expectedCost) < 0.000001,
			`Expected cost to be close to ${expectedCost}, but got ${usage.cost}`
		)
	})
})

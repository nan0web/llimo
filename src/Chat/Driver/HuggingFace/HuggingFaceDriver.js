...
class HuggingFaceDriver extends ChatDriver {
	...
	isThinking(chunk, chunks = [], thinking = false) {
		const [brand, name] = this.model.name.split("/")
		if (name.toLowerCase().startsWith("deepseek-r1")) {
			if (!chunks.length) return true
		}
		return super.isThinking(chunk, chunks, thinking)
	}
